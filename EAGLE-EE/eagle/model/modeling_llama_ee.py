...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # self.predictors = [torch.load('/home/xujiaming/xujiaming/train_machine/PJY/ASPLOS-24-1/llama2-13b-10-models/model'+str(layer_idx)+'.pth').to(torch.float16) for layer_idx in range(config.num_hidden_layers)]
        ...
        self.predictors = [torch.load('/home/xujiaming/xujiaming/train_machine/PJY/ASPLOS-24-1/classifiers-7B/10/model'+str(layer_idx)+'.pth').to(torch.float16) for layer_idx in range(config.num_hidden_layers)]
        self.top_k = 10
        self.depth = 5
    ...
    def forward(
            ...
            draft_tokens_info = None,
            EE_tokens_dict = None,
            draft_index_CSR = None,
            lm_head = None,
        ...
        left_token_local_id_set = set(range(hidden_states.shape[1])) if not init else None
        prune_token_local_id_set = set() if not init else None
        hidden_states_prune = hidden_states
        attention_mask_prune = attention_mask
        position_ids_prune = position_ids
        kv_seq_len = attention_mask.shape[3]-attention_mask.shape[2]
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if not init and len(left_token_local_id_set) < hidden_states_prune.shape[1]:
                attention_mask_prune = torch.zeros(attention_mask.shape[0],attention_mask.shape[1],len(left_token_local_id_set),kv_seq_len+len(left_token_local_id_set),device=attention_mask.device)
                hidden_states_prune = hidden_states[:,list(left_token_local_id_set)]
                attention_mask_prune[:,:,:,kv_seq_len:] = attention_mask[:,:,:,kv_seq_len:][:,:,list(left_token_local_id_set)][:,:,:,list(left_token_local_id_set)]
                position_ids_prune = position_ids[:,list(left_token_local_id_set)]
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states if init else hidden_states_prune,
                    attention_mask=attention_mask if init else attention_mask_prune,
                    position_ids=position_ids if init else position_ids_prune,
                    past_key_value=past_key_value,
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                )
            ...
            # [xjm:] add MLP predictor
            if not init and idx not in [1,3,8,11,15,17,19,21,24,26,28,31]:
                # st_in = time.time()
                hidden_states[:,list(left_token_local_id_set)] = layer_outputs[0]
                
                # hidden_states_tmp = self.norm(hidden_states[:,torch.tensor(list(draft_tokens_list.keys()))])
                hidden_states_tmp = self.norm(hidden_states[:,list(EE_tokens_dict.keys())])
                
                draft_logits = F.linear(hidden_states_tmp, draft_lm_head_weight)
                
                # indices = torch.arange(hidden_states_tmp.shape[1]*4).reshape(hidden_states_tmp.shape[1],4)

                # row_indices = torch.arange(hidden_states_tmp.shape[1]).unsqueeze(1).expand_as(indices)
                draft_logits_new = torch.full((1,hidden_states_tmp.shape[1],self.top_k),float('-inf'),device=hidden_states_tmp.device,dtype=torch.float16)
                
                for row_index in range(len(draft_index_CSR)-1):
                    
                    draft_logits_new[0,row_index,0:(draft_index_CSR[row_index+1]-draft_index_CSR[row_index])] = draft_logits[:,row_index,draft_index_CSR[row_index]:draft_index_CSR[row_index+1]]
                    

                draft_prob = F.softmax(draft_logits_new, dim=-1)
                
                feature = torch.cat([hidden_states_tmp,draft_logits_new,draft_prob],dim=-1).squeeze(0)
                
                
                
                pred = self.predictors[idx](feature)
                # for i in range(len(torch.nonzero(pred.view(-1)>0.5.view(-1)))):
                EE_token_rows_index = torch.nonzero(pred.view(-1)>0.5).view(-1)
                
                if len(EE_token_rows_index) > 0:
                    logits = lm_head(hidden_states_tmp[:,EE_token_rows_index])
                    token = torch.argmax(logits,dim=-1)
                    for token_idx in range(len(EE_token_rows_index)):
                        parent_local_id = list(EE_tokens_dict.keys())[EE_token_rows_index[token_idx]]
                        children_local_id_set = draft_tokens_info[parent_local_id][0]
                        children_global_id_tensor = input_ids[0][list(children_local_id_set)]
                        if token[0,token_idx] in children_global_id_tensor:
                            local_token_id = list(children_local_id_set)[torch.where(children_global_id_tensor == token[0,token_idx])[0]]
                            # print('Start Pruning.......')
                            prune_token_local_id_set = prune_token_local_id_set.union( draft_tokens_info[parent_local_id][1] - draft_tokens_info[local_token_id][1] - {local_token_id})
                            # print('Pruning Over!')
                    print(idx,len(left_token_local_id_set))
            
            
                    left_token_local_id_set = draft_tokens_info[0][1].union({0}) - prune_token_local_id_set
                    if len(left_token_local_id_set) <= self.depth:
                        break
                # tot += time.time() - st_in
